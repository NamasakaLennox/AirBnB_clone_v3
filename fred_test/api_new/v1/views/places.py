...
def search_place():
    ...
    if 'states' in request.get_json() and len(request.get_json()
                                              ['states']) > 0:
        for place in places:
    ...
    if 'cities' in request.get_json() and len(request.get_json()
                                              ['cities']) > 0:
        print(places)
        api_places + [place for place in places if
                      place.city_id in request.get_json()['cities']]

    if 'amenities' in request.get_json() and len(request.get_json()
                                                 ['amenities']) > 0:
        for place in places:
            if all(amenity.id in [amenity.id for amenity in place.amenities]
                    for amenity in request.get_json()['amenities']):
                api_places.append(place)
